...
test('should show correct drop indicator: create-group', async ({ page }) => {
  ...
  await expect(ungroupedIdea2).toHaveAttribute('data-drop-indicator', 'create-group');
...
test('should show correct drop indicator: move-to-group', async ({ page }) => {
  ...
  await expect(group2Idea).toHaveAttribute('data-drop-indicator', 'move-to-group');
...
```typescript
test('should show ghost placeholder when hovering over group', async ({ page }) => {
  await dragIdea(idea);
...
test('should style overlay with shadow and correct colors', async ({ page }) => {
  await dragIdea(idea);
...
```typescript
test('should create new group when dropping ungrouped on ungrouped', async ({ page }) => {
  ...
  await page.waitForFunction(
    (count) => document.querySelectorAll('[data-testid="idea-group"]').length === count + 1,
    initialGroups
  );
...
test('should add ungrouped idea to existing group', async ({ page }) => {
  const groupIdeaCount = await group.locator('[data-testid="idea-card"]').count();

  await dragAndDrop(ungroupedIdea, groupedIdea);

  // Verify idea moved to group
  await expect(group.locator('[data-testid="idea-card"]')).toHaveCount(groupIdeaCount + 1);
...
test('should move idea from one group to another', async ({ page }) => {
  ...
  await expect(group1.locator('[data-testid="idea-card"]')).toHaveCount(group1Count - 1);
  await expect(group2.locator('[data-testid="idea-card"]')).toHaveCount(group2Count + 1);
...
test('should ungroup idea when dropped in ungrouped zone', async ({ page }) => {
  ...
  await expect(group.locator('[data-testid="idea-card"]')).toHaveCount(groupCount - 1);
